...
class AIStoryService: AIStoryServiceProtocol {
    ...
    func saveStory(_ aiStory: AIStoryResponse, voiceModelId: String?) async throws -> Story {
        let story = Story(
            ...
            theme: StoryTheme(rawValue: "") ?? .adventure, // 需要从请求中获取
            style: StoryStyle(rawValue: "") ?? .warm,
...
class MockAIStoryService: AIStoryServiceProtocol {
    ...
    func generateStory(request: AIStoryRequest) async throws -> AIStoryResponse {
        ...
        // 根据主题生成不同的 mock 故事
        let theme = StoryTheme(rawValue: request.theme) ?? .adventure
    ...
    // MARK: - 生成 Mock 故事内容
    private func generateMockStory(theme: StoryTheme, characters: [String], childName: String?) -> (String, String, String, [String]) {
